...
def check_rule(rule: dict, text: str, present: set[str]) -> dict:
    trig = rule["trigger"]
...
def run_rules(rules: list[dict], text: str, present: set[str]) -> dict:
    results = [check_rule(r, text, present) for r in rules]
...
def build_verdict(file: str, areas: list[str], creative_type: str) -> dict:
    ...
    ex = extract(file)
    feats = featuremod.detect(ex["extracted_text"]) if ex["extracted_text"].strip() else []
    ...
    return {
        ...
        "extraction": {
            "extracted_text": ex["extracted_text"],
